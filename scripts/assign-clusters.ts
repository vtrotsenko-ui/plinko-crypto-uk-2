#!/usr/bin/env tsx
/**
 * Cluster-to-site assignment pipeline for the Argentina 1win affiliate project
 * (Agent 3).
 *
 * Input:
 *   - data/clusters.csv   (Agent 2 output)
 *   - data/sites.yaml     (1 site, each with a positioning + a curated page
...
function mandatoryTrustPages(): { slug: string; primary_keyword: string; k: string }[] {
  return [
    ...
    { slug: "/is-1win-legal-in-argentina/", primary_keyword: "is 1win legal in argentina", k: "K7" },
...
async function main(): Promise<void> {
  ...
  for (const site of sites) {
    ...
    for (const { def, cluster } of resolvedPages.get(site.id)!) {
      ...
      const internalLinks =
        def.page_type === "home"
          ...
          : [homeSlug, "/is-1win-legal-in-argentina/", "/responsible-gambling/", "/about-us/"].filter(
              (s) => s !== def.slug
            );
    ...
    for (const trust of mandatoryTrustPages()) {
      ...
      const isLegality = trust.slug === "/is-1win-legal-in-argentina/";
      plan.push({
        site: site.id,
