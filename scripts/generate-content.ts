...
function resolveOfflineContent(page: PlanPage, site: SiteInfo): FinalContent & { ownSectionCount: number } {
  ...
  const fallbackPool: { filename: string; alt: string }[] = [
    { filename: "plinko-odds-diagram-photo.jpg", alt: `Diagram illustrating ${page.primary_keyword} on ${site.name}` },
    { filename: "responsible-gambling-icon.jpg", alt: `Responsible gambling support information on ${site.name}` },
    { filename: "hero-plinko-board.jpg", alt: `Plinko illustration on the ${page.primary_keyword} page of ${site.name}` },
  ];
